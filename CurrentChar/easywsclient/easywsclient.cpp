...
class _RealWebSocket : public easywsclient::WebSocket
{
  public:
    ...
    void poll(std::string& error, int timeout) { // timeout in milliseconds
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin, std::string& error) {
    char host[512];
    int port;
    char path[512];
    if (url.size() >= 512) {
	  char buf[1024]{};
	  snprintf(buf, 1024, "ERROR: url size limit exceeded: Limit is 512, but string length was [%llu]", url.size());
	  error = buf;
      return nullptr;
    }
    if (origin.size() >= 200) {
	  char buf[1024]{};
	  snprintf(buf, 1024, "ERROR: origin size limit exceeded: Limit is 200, but string length was [%llu]", origin.size());
	  error = buf;
      return nullptr;
    }
    if (sscanf(url.c_str(), "ws://%[^:/]:%d/%s", host, &port, path) == 3) {
		...
		char buf[1024]{};
		snprintf(buf, 1024, "ERROR: Could not parse Websocket url: %s", url.c_str());
		error = buf;
        return nullptr;
    }
    ...
    if (sockfd == INVALID_SOCKET) {
		char buf[1024]{};
		snprintf(buf, 1024, "Unable to connect to %s:%d", host, port);
		error = buf;
        return nullptr;
    }
    {
        ...
        if (i == 1023) {
			char buf[2048]{};
			snprintf(buf, 2048, "ERROR: Got invalid status line connecting to: %s", url.c_str());
			error = buf;
        	return nullptr;
        }
        if (sscanf(line, "HTTP/1.1 %d", &status) != 1 || status != 101) {
			char buf[2048]{};
			snprintf(buf, 2048, "ERROR: Got bad status connecting to %s: %s", url.c_str(), line);
        	return nullptr;
        }
	...
	return std::make_shared<_RealWebSocket>(sockfd, useMask);
}
...
WebSocket::pointer WebSocket::from_url(const std::string& url, std::string& error, const std::string& origin) {
    return ::from_url(url, true, origin, error);
}

WebSocket::pointer WebSocket::from_url_no_mask(const std::string& url, std::string& error, const std::string& origin) {
    return ::from_url(url, false, origin, error);
}
